...
# Versão: 1.0 (Refatorado Base v9.4)
# "Agora com versionamento e nomes de arquivos dinâmicos."
# ==============================================
...
# Controle de Interatividade e Nome Personalizado
...
# Setup Inicial de Timestamp (Nomes de arquivos serão definidos no main após processar argumentos)
mkdir -p logs
...
show_help() {
    ...
    echo -e "  ${GREEN}-o <texto>${NC}     Nome opcional para identificar o teste no arquivo (Ex: TESTE01)"
    ...
    echo -e ""
    echo -e "${CYAN}----------------------------------------------------------${NC}"
    echo -e "🔗 ${GRAY}Repositório Oficial & Documentação:${NC}"
    echo -e "   👉 ${YELLOW}github.com/flashbsb/diagnostico_dns${NC}"
    echo -e "${CYAN}----------------------------------------------------------${NC}"
    echo -e ""
}
...
init_log_file() {
    ...
    local sys_user=$(whoami); local sys_host=$(hostname); local sys_kernel=$(uname -r)
    {
        ...
        echo "  Timeout Global : ${TIMEOUT}s"
        echo "  Sleep Interval : ${SLEEP}s"
        echo "  Conn Check     : ${VALIDATE_CONNECTIVITY}"
        echo "  IP Version     : ${IP_VERSION}"
        echo "  Dig Options    : ${DEFAULT_DIG_OPTIONS}"
        echo "  Rec Options    : ${RECURSIVE_DIG_OPTIONS}"
        echo "  Files          : $FILE_DOMAINS / $FILE_GROUPS"
        if [[ -n "$CUSTOM_NAME" ]]; then
            echo "  Tag Nome       : $CUSTOM_NAME"
        fi
        echo ""
    } > "$LOG_FILE_TEXT"
}
        ...
        <div class="config-section">
             ...
             <table class="config-table">
                <tbody>
                    ...
                    <tr><th>Tag Opcional</th><td>${CUSTOM_NAME:-"Nenhuma"}</td></tr>
                </tbody>
             </table>
        </div>
EOF
...
load_dns_groups() {
    ...
    log_entry "INFO: Carregados $count grupos DNS do arquivo $FILE_GROUPS"
}
...
run_ping_diagnostics() {
    ...
    log_section "INICIANDO TESTES DE LATENCIA (PING)"
...
process_tests() {
    ...
    log_section "INICIANDO TESTES DNS"
    local test_id=0
...
main() {
    ...
    # Parsing de Argumentos (incluindo -o para o nome personalizado)
    while getopts ":n:g:o:lhy" opt; do
        case ${opt} in
    ...
    # Construção dos nomes dos arquivos baseados nos argumentos (v1.0)
    local NAME_PART=""
    [[ -n "$CUSTOM_NAME" ]] && NAME_PART="_${CUSTOM_NAME}"
    
    # Redefine variáveis globais de saída com o novo padrão
    HTML_FILE="logs/dnsdiag_v${SCRIPT_VERSION}${NAME_PART}_${TIMESTAMP}.html"
    LOG_FILE_TEXT="logs/dnsdiag_v${SCRIPT_VERSION}${NAME_PART}_${TIMESTAMP}.log"
    ...
    # Finaliza o log de texto e adiciona a versão no rodapé
    ...
    echo -e "\n${GREEN}=== DIAGNÓSTICO CONCLUÍDO (v$SCRIPT_VERSION) ===${NC}"
